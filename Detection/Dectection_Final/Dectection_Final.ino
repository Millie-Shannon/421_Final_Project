...
//Slide Switch + (Mode 1): Sitting Case 
  ...
  if (mode == 1 && nosit > 30 && sensorValue < 100) {   // user must be standing for at least 
    tsit = 0;                                           // 30 seconds before alarms / timer reset
  }

// notification system that user has been sitting for too long
// 2 hours is 7200 seconds
  if (mode == 1 && tsit > 7200 && ledState == 0 && sensorValue > 100) {     // if seated for too long,
  ...
  if (mode == 1 && tsit > 7200 && ledState == 1 && sensorValue > 100) {     // (if lights on, they will turn off)
...
x = tsit % 5;                                       // after every 5 cycles of blinking lights
                                                                           // if user doesn't get up within two minutes of light alert
  if (mode == 1 && x == 0 && tsit > 7320 && sensorValue > 100) {           // tones will play serving as an alarm
